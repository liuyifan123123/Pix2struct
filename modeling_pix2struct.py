...
class Pix2StructLayerNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        variance = hidden_states.to(ops.float32).pow(2).mean(-1, keepdim=True)
        hidden_states = hidden_states * ops.rsqrt(variance + self.variance_epsilon)

        # convert into half-precision if necessary
        if self.weight.dtype in [ops.float16, ops.bfloat16]:
...
class Pix2StructVisionAttention(nn.Module):
    ...
    def forward(
        ...
        scores = ops.matmul(query_states, key_states.transpose(3, 2))
        ...
        if position_bias is None:
            ...
            if attention_mask.dim() == 2:
                position_bias = position_bias + attention_mask[:, None, None, :].to(position_bias)
            else:
                # (batch_size, n_heads, seq_length, key_length)
                position_bias = position_bias + attention_mask.to(position_bias)
            position_bias = 1 - position_bias

        position_bias_masked = position_bias.masked_fill(position_bias == 1, ops.finfo(scores.dtype).min)
        ...
        attn_output = attn_output.transpose(1, 2).view(batch_size, -1, self.inner_dim)
...
class Pix2StructVisionMlp(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if (
            ...
            and self.wo.weight.dtype != mindspore.int328
        ):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Pix2StructLayerNorm):
            ...
            module.weight.data = nn.init.trunc_normal_(
                module.weight.data.to(ops.float32), mean=0.0, std=self.config.initializer_range
...
PIX2STRUCT_VISION_INPUTS_DOCSTRING = r"""
    Args:
        flattened_patches (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_channels x patch_height x patch_width)`):
...
class Pix2StructTextBlock(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.dtype == ops.float16 and ops.isinf(hidden_states).any():
...
PIX2STRUCT_INPUTS_DOCSTRING = r"""
    Args:
        flattened_patches (`mindspore.Tensor` of shape `(batch_size, seq_length, hidden_size)`):
        ...
        for layer_past_states in past_key_values:
            ...
            for layer_past_state in layer_past_states:
                ...
                reordered_layer_past_states = reordered_layer_past_states + (
                    layer_past_state.index_select(0, beam_idx.to(layer_past_state)),
                )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if decoder_attention_mask is None:
            decoder_attention_mask = ops.ones_like(input_ids).to(input_ids)
